...
export default class AxisPad extends Component {
    ...
    centerPosition(pageX, pageY) {
        let ref; 
        if (typeof this.handlerElement.measure === 'function'){ref = this.handlerElement;}
        else{ref = this.handlerElement.getNode();}
    ...
    setPosition(pageX, pageY, after) {
        let ref; 
        if (typeof this.wrapperElement.measure === 'function'){ref = this.wrapperElement;}
        else{ref = this.wrapperElement.getNode();}

        let { dx, dy } = this.state;
        ref.getNode().measure((fx, fy, width, height, px, py) => {
            const cx = px + width / 2;
